...
import { ArrowLeft, Building2, Users, LayoutGrid, Settings, CalendarOff, BarChart3, Palmtree } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useAuth } from '../contexts/AuthContext';
import { organizationService, leaveService } from '../utils/services';
import type { OrganizationDetail, OrgRole, LeaveBalance, OrgDepartment, OrgJobGroup } from '../types';
...
export function OrganizationDetailPage() {
  ...
  const fetchOrg = useCallback(async () => {
    ...
    try {
      ...
      const [balances, membersData, depts, jgs] = await Promise.all([
        leaveService.getMyBalance(orgId).catch(() => [] as LeaveBalance[]),
        organizationService.getMembers(orgId, { size: 200 }).catch(() => null),
        organizationService.getDepartments(orgId).catch(() => [] as OrgDepartment[]),
        organizationService.getJobGroups(orgId).catch(() => [] as OrgJobGroup[]),
      ]);
      setMyLeaveBalances(balances);
      setDepartments(depts);
      setJobGroups(jgs);
      if (membersData && currentUser) {
        const me = membersData.content.find((m) => m.user.id === currentUser.id);
    ...
    <div className="min-h-screen bg-bridge-dark">
      <div className="max-w-6xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-xl font-bold text-foreground tracking-tight">{org.name}</h1>
                  <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${ROLE_BADGE_STYLES[myRole]}`}>
                    {myRole}
                  </span>
                </div>
          ...
          <div className="flex items-center gap-3">
            {myLeaveBalances.length > 0 && (
              <div className="flex items-center gap-2 bg-bridge-obsidian rounded-xl border border-foreground/[0.05] px-3 py-2">
                <Palmtree size={14} className="text-emerald-500 shrink-0" />
                <div className="flex items-center gap-2">
                  {myLeaveBalances.map((b) => (
                    <div key={b.id} className="flex items-center gap-1.5">
                      <span className="text-[11px] text-muted-foreground">{b.policy_name}</span>
                      <span className="text-[13px] font-bold text-foreground">{b.remaining}</span>
            ...
            {currentUser && (
              ...
              >
                {currentUser.profile_image ? (
                  ...
                  <div className="w-9 h-9 rounded-full bg-bridge-accent/20 ring-2 ring-bridge-accent/30 flex items-center justify-center">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === 'dashboard' && <OrgDashboardTab orgId={orgId!} role={myRole} />}
            {activeTab === 'members' && <OrgMembersTab orgId={orgId!} myRole={myRole} myUserId={currentUser?.id || ''} />}
