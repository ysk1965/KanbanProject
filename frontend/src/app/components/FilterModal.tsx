  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg bg-bridge-dark text-foreground border-bridge-border p-0 gap-0">
        ...
        <div className="px-4 py-3 border-b border-bridge-border">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-9 bg-bridge-surface border-bridge-border text-foreground placeholder:text-zinc-400 h-9"
            />
            {filters.keyword && (
              <button
        ...
        {/* 탭 네비게이션 */}
        <div className="flex border-b border-bridge-border px-2 overflow-x-auto">
          {tabs.map((tab) => (
            <button
              key={tab.id}
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                          filters.tags.includes(tag.id)
                            ? 'ring-2 ring-white ring-offset-2 ring-offset-bridge-dark'
                            : 'hover:opacity-80'
                        }`}
        ...
        {/* 선택된 필터 요약 & 액션 */}
        <div className="px-4 py-3 border-t border-bridge-border bg-bridge-surface">
          {/* 선택된 필터 표시 */}
          {isFilterActive && (
