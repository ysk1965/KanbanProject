...
export function AdminChurnTab() {
  ...
  const [inactiveUsers, setInactiveUsers] = useState<InactiveUserList | null>(null);
  const [trialDropout, setTrialDropout] = useState<TrialDropoutAnalysis | null>(null);
  ...
  const loadInactiveUsers = async () => {
    try {
      const data = await adminService.getInactiveUsers(inactivePeriod, inactivePage, 20);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-foreground mb-2">{t('admin.churn.title')}</h2>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr>
                  ...
                  <th className="text-center py-2 px-2 text-slate-400 font-medium">{t('admin.churn.signups')}</th>
                  {Array.from({ length: Math.min(retention.cohorts[0]?.retention.length || 0, 9) }, (_, i) => (
                    <th key={i} className="text-center py-2 px-1 text-slate-400 font-medium">
              ...
              <tbody>
                {retention.cohorts.map((cohort) => (
                  <tr key={cohort.cohort_week} className="border-t border-foreground/[0.05]">
                    <td className="py-1.5 px-2 text-slate-400">{formatWeek(cohort.cohort_week)}</td>
                ...
                <tr className="border-t-2 border-foreground/[0.12]">
                  <td className="py-1.5 px-2 text-foreground font-bold">{t('admin.churn.average')}</td>
      ...
      {trialDropout && trialDropout.total_expired_trials > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <BarChart data={trialDropout.dropout_by_day}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number, name: string) => {
                      if (name === 'count') return [value, t('admin.churn.dropoutCount')];
                      return [value, name];
                    }}
                  />
          ...
          <div className="bg-bridge-obsidian rounded-2xl border border-foreground/[0.08] p-5">
            <h3 className="text-sm font-bold text-foreground mb-4">{t('admin.churn.actionsBeforeDropout')}</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [value, t('admin.churn.boardCount')]}
                  />
      ...
      {activityTrends && activityTrends.weekly_activity.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {activityTrends.activity_change_rate !== 0 && (
                  <p className={`text-xs mt-1 flex items-center gap-1 ${
                    ...
                    {t('admin.churn.changeRate')}: {activityTrends.activity_change_rate > 0 ? '+' : ''}{activityTrends.activity_change_rate}%
                  </p>
                )}
              </div>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={activityTrends.weekly_activity}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                  <XAxis dataKey="week" tickFormatter={formatWeek} tick={{ fill: '#94a3b8', fontSize: 11 }} />
                  <YAxis yAxisId="left" tick={{ fill: '#94a3b8', fontSize: 11 }} />
                  <YAxis yAxisId="right" orientation="right" tick={{ fill: '#94a3b8', fontSize: 11 }} />
                  <Tooltip contentStyle={tooltipStyle} labelFormatter={formatWeek} />
                  <Legend />
                  <Line
                    yAxisId="left"
                    type="monotone"
          ...
          {/* Feature Usage */}
          <div className="bg-bridge-obsidian rounded-2xl border border-foreground/[0.08] p-5">
            <h3 className="text-sm font-bold text-foreground mb-4">{t('admin.churn.featureUsage')}</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={activityTrends.feature_usage.slice(0, 10)}>
                  ...
                  />
                  <Legend />
                  <Bar dataKey="count" name={t('admin.churn.totalActions')} fill="#6366F1" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="unique_users" name={t('admin.churn.uniqueUsers')} fill="#2DD4BF" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {inactiveUsers && (
        <div className="bg-bridge-obsidian rounded-2xl border border-foreground/[0.08] p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-foreground">{t('admin.churn.inactiveUsers')}</h3>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {inactiveUsers.users.map((user) => (
                  <tr key={user.id} className="border-b border-foreground/[0.04] hover:bg-foreground/[0.02] transition-colors">
...
function MetricCard({
  ...
  const bgColor = color === 'amber' ? 'bg-amber-500/10' : color === 'orange' ? 'bg-orange-500/10' : 'bg-red-500/10';
