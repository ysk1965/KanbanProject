...
/** 헤더 + 마일스톤·이벤트 밴드 + 내 행이 여유 있게 들어가는 높이 */
...
/**
 * 내 워크로드 — 일정 탭의 ScheduleResourceView + 배치 인박스를 한 카드에 담는다.
 *
 * 왼쪽(간트)은 이미 배치된 일, 오른쪽(트레이)은 아직 자리가 없는 일이다.
 * 트레이 카드를 내 행의 날짜 칸에 떨구면 그 날짜로 시작·마감이 잡히고 바가 생긴다.
...
export function MyWorkloadWidget({
  ...
  // 배치 후 간트·트레이를 함께 다시 그리기 위한 자체 신호.
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row">
        {myMembers.length === 0 ? (
          ...
          <>
            <div className="flex-1 min-h-0 min-w-0 flex">
              ...
              >
                <ScheduleResourceView
                  ...
                  onDropChecklist={(item, targetDate, targetAssigneeId) => {
            ...
            <div className="flex-none h-[190px] lg:h-auto lg:w-[240px]">
              <PlacementTray
                boardId={boardId}
                userId={userId}
                refreshTrigger={childRefreshTrigger}
