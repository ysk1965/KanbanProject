...
import { Clock, ChevronDown, ChevronUp, CheckCircle, Loader2 } from 'lucide-react';
...
function utcToLocal(utcHour: number, utcMinute: number, timezone: string): { hour: number; minute: number } {
  try {
    const now = new Date();
    const utcDate = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate(), utcHour, utcMinute));
    const localStr = utcDate.toLocaleString('en-US', { timeZone: timezone, hour12: false, hour: '2-digit', minute: '2-digit' });
...
export function StandupConfigPanel({ boardId, isAdmin, canAccessSlack, hasSlack }: StandupConfigPanelProps) {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      if (data) {
        ...
        const local = utcToLocal(data.send_hour_utc, data.send_minute_utc, data.timezone);
        setHour(local.hour);
  ...
  const saveConfig = useCallback(async (
    newEnabled: boolean, newHour: number, newMinute: number,
    newTimezone: string, newLanguage: string
  ) => {
    setIsSaving(true);
    setSaved(false);
    try {
    ...
    <div className="mx-3 mb-2 bg-white/[0.03] rounded-xl border border-white/10 overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-1">
          {isSaving && <Loader2 size={10} className="text-slate-400 animate-spin" />}
          ...
          {enabled && hasSlack && (
            <>
              ...
              <div>
                ...
                >
                  {TIMEZONES.map((tz) => (
                    <option key={tz.value} value={tz.value} className="bg-bridge-obsidian">
                      {tz.label}
                    </option>
                  ))}
                </select>
              ...
              {config?.last_sent_at && (
                <div className="text-[9px] text-slate-500">
                  {t('standupConfig.lastSent')}: {formatRelativeTime(config.last_sent_at)}
                </div>
              )}
            </>
          )}
