...
export function parseUTCDate(
  ...
  // 파싱 실패 시 Invalid Date 대신 null 반환 (다운스트림 format() 크래시 방지)
...
export function getDDay(dueDate: string | null | undefined): DdayInfo {
  ...
  if (!isValid(parsed)) return { text: "", diff: 0, urgency: "none" };
  const today = startOfDay(new Date());
  const due = startOfDay(parsed);
  const diff = differenceInCalendarDays(due, today);

  if (diff < 0)
    return { text: `D+${Math.abs(diff)}`, diff, urgency: "overdue" };
...
/** 마감일 기준 그룹 분류 */
export function getDeadlineGroup(
  dueDate: string | null | undefined,
): DeadlineGroup {
  if (!dueDate) return "noDate";
  const parsed = parseISO(dueDate);
  if (!isValid(parsed)) return "noDate";
  const today = startOfDay(new Date());
  const due = startOfDay(parsed);
  const diff = differenceInCalendarDays(due, today);
