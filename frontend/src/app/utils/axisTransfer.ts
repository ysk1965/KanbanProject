...
export interface AxisDropDetail {
   ...
   *
   * 그래서 to === "timeblock"인 이벤트에서 이 값의 유무가 곧 단계 표시다:
   *   없음 = "바를 여기에 놓았다"(마우스 기반 소스가 쏘는 알림, 타임블록이 받아서 처리)
   *   있음 = "블록까지 만들었다"(타임블록이 되쏘는 알림, 항목의 주인이 받아서 처리)
   * 타임블록이 앞의 것만 받아야 자기 이벤트를 되받아 블록을 두 번 만들지 않는다.
   */
  targetDate?: string;
}
