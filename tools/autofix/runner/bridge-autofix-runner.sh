...
# 자식(autofix-once.sh)의 하트비트에도 실린다. 하트비트가 버전 없이 나가면 서버 기록이
# 지워지고, 작업이 도는 동안에는 claim이 없어 아무도 복구하지 않는다 — 그 사이 도크가
# "스크립트가 낡았습니다"를 띄운다. (선언 줄은 CI가 파싱하므로 export는 따로 둔다.)
export RUNNER_CONTRACT
