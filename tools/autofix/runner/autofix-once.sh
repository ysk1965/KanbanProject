...
if [ -n "$LOCALE_SOURCE_URL" ]; then
  ...
  if [ -n "$LOCALE_SOURCE_MIRROR" ]; then
    if [ -f "$LOCALE_SOURCE_MIRROR" ]; then   # cwd는 이미 PROJECT_DIR이다
      # 사본이 저장소 밖이면 에이전트에게 그 폴더를 열어 줘야 Read가 닿는다.
      #
      # 밖에 두는 이유: 시트 스냅샷을 작업 트리 안에 두면 git status가 더러워져 사전 점검이
      # 막히고, untracked라 git clean -fd 가 매 실행마다 지운다. 첨부 자료(MATERIALS_DIR)를
      # 밖에 두는 것과 같은 이유다.
      case "$LOCALE_SOURCE_MIRROR" in
        /*) LOCALE_MIRROR_ADD_DIR="$(dirname "$LOCALE_SOURCE_MIRROR")" ;;
      esac

      # --add-dir 은 파일이 아니라 폴더를 연다. 그래서 사본을 러너 홈에 그냥 두면
      # 같은 폴더의 runner.conf(BRIDGE_TOKEN·저장소 경로)까지 에이전트가 읽을 수 있다.
      # 전용 하위 폴더를 쓰게 강제한다 — 설정 실수 한 번이 토큰 유출이 되면 안 된다.
      if [ -n "$LOCALE_MIRROR_ADD_DIR" ] && [ -e "$LOCALE_MIRROR_ADD_DIR/runner.conf" ]; then
        fail "LOCALE_SOURCE_MIRROR 가 runner.conf 와 같은 폴더에 있습니다 ($LOCALE_MIRROR_ADD_DIR).
그 폴더를 통째로 에이전트에게 열게 되므로 토큰이 노출됩니다. 전용 하위 폴더로 옮기세요
(예: $LOCALE_MIRROR_ADD_DIR/locale/$(basename "$LOCALE_SOURCE_MIRROR"))"
      fi
      log "로케일 원본 사본 — $LOCALE_SOURCE_MIRROR"
      # 스냅샷이 언제 것인지는 로그에 남겨야 한다. 값이 시트와 어긋났을 때 "에이전트가 틀렸다"와
      # "사본이 낡았다"를 구분하는 유일한 단서다.
      head -1 "$LOCALE_SOURCE_MIRROR" | grep -q '스냅샷' \
        && log "  $(head -1 "$LOCALE_SOURCE_MIRROR" | sed 's/^# *//')"
    else
...
한 줄이 항목 하나이고, 탭으로 구분된 열은 다음과 같다:

  tab · id · 분류 · 서브분류 · key · kr · en

보고에 옮겨 적을 값이 한 줄에 다 있다. 줄 첫 칸의 tab이 원본의 어느 탭인지이고, 이것도
보고에 적어야 한다 — 원본에는 정본 탭이 둘(Default, Dialogue)이라 키만으로는 사람이 찾지 못한다.
파일 맨 위 주석에 이 사본을 언제 떠 왔는지가 적혀 있다. 시트가 그 뒤에 바뀌었을 수 있으므로,
이슈가 말하는 현재 문구와 사본의 값이 어긋나면 그 사실을 보고에 적어라(사본이 낡았다는 뜻이다).
...
원본 수정 보고 — 마지막에 이 형식 그대로 출력한다. 사람이 이걸 들고 원본을 찾는다:
  ...
  [로컬라이즈 원본 수정 필요]
  - 탭: <원본의 어느 탭인가. 사본을 썼다면 그 줄의 첫 칸 값. 모르면 "모름">
...
문구 자체가 원본에 아예 없어 새로 만들어야 하면 형식이 다르다. "현재"가 없기 때문이다:

  [로컬라이즈 원본 추가 필요]
  - 제안 키: <규약에 맞는 새 키 이름. 같은 화면·기능이 쓰는 접두사 계열을 따른다>
  - 대역: <#id를 어느 구간에 넣어야 하는지. 같은 기능의 기존 항목 id를 근거로 든다>
  - 값(kr): <넣을 문구>
  - 쓰이는 곳: <어느 코드가 이 키를 불러야 하는지. 파일:줄>
  - 근거: <왜 기존 키로 안 되는지. 무엇을 찾아봤는데 없었는지>

키를 새로 만드는 것은 원본을 쥔 사람의 판단이다. 제안만 하고 코드에 키를 심지 마라 —
원본에 없는 키를 코드가 부르면 그 화면은 토큰이 날것으로 노출된 채 배포된다.
PROMPT_SRC
}

# 로케일과 무관한 「변경 없음」에도 산출물이 있어야 한다.
#
# 위 보고 형식은 원본이 저장소 밖일 때만 나간다(emit_locale_source). 그런데 고칠 수 없는 이유는
# 그 밖에도 많다 — 기대 동작이 확정되지 않았거나, 재현이 서버 상태에 달렸거나, 원인이 이 저장소가
# 아닌 경우다. 그때 화면에 남는 것은 "자동으로 고칠 수 없다고 판단했습니다" 한 문장뿐인데,
# 그것은 사람이 다음에 무엇을 할지 아무것도 말해주지 않는다. 판단한 이유와 넘길 거리를 남긴다.
emit_no_change_report() {
  cat <<'PROMPT_SRC'

변경 없이 끝낼 때 — 아무것도 고치지 않기로 했다면 마지막에 반드시 이 형식을 출력한다.
(위 로컬라이즈 보고를 남겼다면 그것으로 충분하다. 이 블록은 그 외의 모든 경우에 쓴다.)

  [사람이 해야 할 일]
  - 이유: <왜 이 저장소에서 고칠 수 없는지 한 줄>
  - 확인한 것: <무엇을 보고 그렇게 판단했는지. 파일·키·콘솔 로그 등 근거>
  - 필요한 것: <누가 무엇을 정해주면 자동으로 고칠 수 있는지. 없으면 "코드 밖 문제">

"고칠 수 없다"만 남기고 끝내지 마라. 그 한 줄은 읽는 사람에게 아무 일도 시키지 못하고,
결국 사람이 처음부터 다시 조사하게 만든다 — 네가 이미 알아낸 것을 버리는 셈이다.
PROMPT_SRC
}
...
6. 문구가 테이블에 없으면 키를 새로 만들지 말고, 없다는 사실과 필요한 문구를 보고하고 끝낸다
   (보고 형식은 아래 「변경 없이 끝낼 때」를 따른다 — 형식을 벗어난 설명은 화면에 닿지 않는다)
...
run_with_timeout "$TIMEOUT_MINUTES" \
  claude -p "$(cat "$PROMPT_FILE")" \
    ...
    ${LOCALE_MIRROR_ADD_DIR:+--add-dir "$LOCALE_MIRROR_ADD_DIR"} \
