...
## 5-1. 매 건의 출발선 — 정리하고, 최신에서 시작한다

러너는 작업을 받을 때마다 이 순서를 밟는다. **지난 건이 무엇을 남겼든 이번 건에는 닿지 않는다.**

| 순서 | 하는 일 | 남는 것 |
|------|---------|---------|
| 1 | 에디터가 재직렬화한 `.asset`·`.prefab`·`.unity` 원복 | — |
| 2 | 잔재가 있으면 추적 파일 수정을 `autofix-rescue` stash로 회수 | `git stash list` |
| 3 | `reset --hard` + `clean -fd` (`Library`·`Temp`·`obj` 제외) | — |
| 4 | `git fetch origin <base_ref>` → `checkout -B <작업브랜치> origin/<base_ref>` | 항상 최신 base |
| 5 | 오래된 로컬 `autofix/*` 브랜치·원격 추적 ref 정리, rescue stash 개수 정리 | 최근 것만 |
| 6 | (작업 종료 후) 다시 `reset --hard` + `clean`, 로컬 base 브랜치를 `origin`에 맞춤 | 깨끗한 최신 트리 |

**로컬 브랜치가 얼마나 낡았든 상관없다.** 4단계가 `origin`에서 기준을 새로 받아 거기서 브랜치를
끊기 때문에, 이 맥에서 `git pull`을 며칠 안 했더라도 러너가 고치는 것은 항상 최신 코드다.
6단계가 로컬 base 브랜치까지 맞추는 것은 러너를 위해서가 아니라 **작업 사이에 Editor가 열고 있는
것이 그 브랜치**여서다 — 낡은 채로 두면 사람이 보는 화면과 러너가 고치는 코드가 어긋나고,
밀린 재임포트가 다음 작업 도중에 몰려 그 건의 시간을 잡아먹는다.

> **왜 「더러우면 실패」가 아니라 「치우고 시작」인가.** 락을 잡은 뒤에도 트리가 더럽다는 것은
> "지금 누가 쓰고 있다"가 아니라 대부분 **이전 실행이 정리 못 하고 죽은 흔적**(SIGKILL·강제종료)이다.
> 그런데 예전 동작은 그 대가를 엉뚱한 다음 작업에 물렸다 — 실패한 작업의 대상은
> `existsActiveForIssue` 때문에 사람이 취소하기 전까지 다시 큐에 담기지 않으므로, 남이 남긴 파일
> 한 줄에 멀쩡한 이슈가 하나 타 버린다.

### ⚠️ 이 맥에서 손으로 작업할 때

작업 트리는 **작업을 받는 순간 비워진다.** 추적 파일의 수정은 stash로 건져 두지만
(`git stash list` → `git stash pop`), ignore되지 않은 **untracked 파일은 복구되지 않는다.**
이 맥에서 직접 뭔가 만들 거라면 커밋하거나 다른 클론에서 할 것.

`Library`·`Temp`·`obj`는 어느 단계에서도 지우지 않는다. 지우면 다음 임포트에 수십 분이 든다.
보관 기간은 `runner.conf`의 `AUTOFIX_BRANCH_KEEP_DAYS`(기본 7일)·`AUTOFIX_RESCUE_STASH_KEEP`
(기본 5개)로 조정한다.
...
4. 정리 단계 후 작업 트리가 깨끗한가. 일부러 더럽혀 두고 한 번 더 돌려, **막히지 않고
   치우고 시작하는지**와 그 수정이 `git stash list`에 남는지 확인할 것 (5-1절)
...
| 맥에서 손으로 만들던 수정이 사라짐 | `git stash list`의 `autofix-rescue` 항목 (5-1절) |
