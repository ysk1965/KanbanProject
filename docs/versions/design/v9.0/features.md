...
Feature (피쳐 카드)
    ...
    └── Task (태스크 카드)
            ...
            └── Checklist (체크리스트)
                    ...
                    ├── 완료 여부
                    │
                    ├── Daily Checklist (데일리 체크리스트 할당)  ← v9.0 추가
                    │       │
                    │       ├── 할당 날짜
                    │       ├── 담당자별 우선순위 (position)
                    │       └── 보드 연결
...
| Daily Checklist | 데일리 체크리스트 뷰 | 체크리스트 항목 추가 | Premium only | ← v9.0 추가
...
## 9. 데일리 체크리스트 시스템 (Premium) ← v9.0 신규

### 9.1 개요

체크리스트 항목을 **날짜별로 할당**하고, **멤버별 우선순위**로 관리하는 일일 작업 관리 시스템

### 9.2 핵심 개념

| 개념 | 설명 |
|------|------|
| **날짜 할당** | 체크리스트 항목을 특정 날짜에 할당 |
| **멤버별 컬럼** | 팀원별로 할당된 항목을 컬럼 형태로 표시 |
| **우선순위** | 드래그-드롭으로 position 기반 정렬 |
| **읽기 전용 모드** | 과거 날짜는 수정 불가 (조회만 가능) |

### 9.3 UI 구조

```
┌────────────────────────────────────────────────────────────────────┐
│ 데일리 체크리스트                 [< 2026.01.17 >] [오늘]          │
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐             │
│  │   김개발      │  │   박디자인    │  │   이기획      │             │
...
│                                                                    │
│  마일스톤 필터: [전체 ▼]                                           │
│  검색: [_________________________________]                          │
│                                                                    │
│  ─────────────────────────────────────────────────────────────────  │
│                                                                    │
│  Feature: 사용자 인증                                              │
...
│  ─────────────────────────────────────────────────────────────────  │
│                                                                    │
│  + 새 체크리스트 항목 추가: [___________________] [추가]            │
│                                                                    │
├────────────────────────────────────────────────────────────────────┤
│                                           [취소]    [추가]         │
└────────────────────────────────────────────────────────────────────┘
...
|------|------|
| **board_id** | 소속 보드 |
| **checklist_item_id** | 연결된 체크리스트 (nullable) |
| **assignee_id** | 담당자 |
| **assigned_date** | 할당 날짜 |
| **position** | 담당자/날짜별 우선순위 |
| **title** | 제목 (checklistItem 삭제 시에도 유지) |

### 9.6 핵심 기능

| 기능 | 설명 |
|------|------|
| **날짜 네비게이션** | 이전/다음 날짜로 이동, 오늘로 돌아가기 |
| **드래그-드롭 정렬** | 같은 컬럼 내에서 우선순위 변경 |
| **항목 추가** | 기존 체크리스트 선택 또는 새로 생성하며 추가 |
| **항목 제거** | 데일리 체크리스트에서만 제거 (원본 체크리스트 유지) |
| **완료 표시** | 원본 체크리스트의 완료 상태 표시 (읽기 전용) |
| **Feature 색상** | 체크리스트가 속한 Feature의 색상 표시 |

### 9.7 제약 조건

| 조건 | 설명 |
|------|------|
| **중복 방지** | 같은 보드, 같은 체크리스트, 같은 날짜에 중복 추가 불가 |
| **과거 날짜** | 읽기 전용 (수정/추가/삭제 불가) |
| **담당자 기준** | 체크리스트의 담당자(assignee) 컬럼에만 추가 가능 |
| **ChecklistItem 삭제 시** | 연결 해제되지만 DailyChecklist 레코드는 title과 함께 유지 |
...
| Daily Checklist 생성 | 데일리 체크리스트에서 항목 추가 (Premium only) | ← v9.0 추가
| 완료 반영 | Done 이동 시 자동 |
...
| Daily Checklist 과거 날짜 | 읽기 전용 | ← v9.0 추가
...
| 버전 | 날짜 | 주요 변경 |
|------|------|----------|
| v7.0 | 2026-01 | Task.assignee 제거, 관리 대시보드 추가 |
| v8.0 | 2026-01 | 가중치 시스템, 통계 분석, 설정 시스템 추가 |
