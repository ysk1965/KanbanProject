...
### 3.6 데일리 체크리스트 아이템 ← v9.0 추가

```tsx
// 체크리스트 아이템 (드래그 가능)
<div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-white/10
  hover:border-white/20 cursor-grab active:cursor-grabbing transition-all">
  {/* 드래그 핸들 */}
  ...
  <div className="flex-1">
    <span className="text-white text-sm">체크리스트 제목</span>
    <span className="text-slate-500 text-xs ml-2">(Task 이름)</span>
  </div>
...
// 멤버 컬럼
<div className="flex flex-col bg-bridge-obsidian rounded-xl border border-white/10 p-4 min-w-[280px]">
  {/* 컬럼 헤더 */}
  <div className="flex items-center gap-3 mb-4">
    <img src="..." className="w-8 h-8 rounded-full" />
    <span className="text-white font-medium">멤버 이름</span>
    <span className="text-slate-500 text-sm">(3 items)</span>
  </div>
  ...
  {/* 추가 버튼 */}
  <button className="mt-4 flex items-center justify-center gap-2 py-2
...
### 4.5 데일리 체크리스트 상태 ← v9.0 추가

| 상태 | 색상 | 조건 |
|------|------|------|
| 완료 | 초록 (`text-emerald-400`) | checklist.is_completed = true |
| 미완료 | 흰색 (`text-white`) | checklist.is_completed = false |
| 읽기 전용 | 회색 (`text-slate-500`) | 과거 날짜 |
| 연결 해제됨 | 회색 테두리 (`border-dashed`) | checklist_item_id = null |
...
### 6.4 드래그-드롭 애니메이션 ← v9.0 추가

```tsx
// dnd-kit 드래그 오버레이
<DragOverlay>
  <div className="opacity-80 shadow-2xl transform scale-105">
    {/* 드래그 중인 아이템 */}
  </div>
</DragOverlay>
...
| 시간 | `Clock` | h-4 w-4 |
| 드래그 핸들 | `GripVertical` | h-4 w-4 | ← v9.0 추가
| 이전/다음 | `ChevronLeft`, `ChevronRight` | h-5 w-5 |
...
| v9.0 | 2026-01 | 데일리 체크리스트 컴포넌트 스타일, 드래그-드롭 애니메이션 추가 |
