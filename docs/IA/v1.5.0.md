...
│   │   │   ├── report/          # AI 리포트 생성/관리 (NEW v1.5)
│   │   │   └── standup/         # 데일리 스탠드업 자동 발송 (NEW v1.5)
│   │   └── global/              # 전역 설정 (27개 파일)
│   │       ├── config/          # Spring 설정 (8개 파일)
...
│   ├── src/main/resources/
│   │   ├── application.yml
│   │   └── db/migration/        # Flyway 마이그레이션 (V1~V16)
│   ├── build.gradle
│   └── Dockerfile
│
...
| i18n | `i18n/` | 다국어 지원 10개 언어 (ko, en, ja, zh, zh-TW, hi, vi, es, pt-BR, th) (NEW v1.5) |
| Landing | `components/landing/` | 랜딩 페이지 + ComparisonPage (NEW v1.5) |
...
| Feature | `domain/feature/` | 피쳐 카드 CRUD, 진행률 (priority 필드 제거, v1.5) |
...
| Subscription | `domain/subscription/` | 구독/결제 관리, PricingPlan (Trial 3일) |
...
| Report | `domain/report/` | AI 리포트 생성/관리 (WeeklyReport, ReportType, ReportController, ReportService, ReportAIService, ReportRepository, DTOs) (NEW v1.5) |
| Standup | `domain/standup/` | 데일리 스탠드업 자동 발송 (DailyStandupConfig, DailyStandupController, DailyStandupService, DailyStandupScheduler, DailyStandupConfigRepository, DTOs) (NEW v1.5) |

### Backend Global 설정

| 파일 | 경로 | 설명 |
|------|------|------|
| SecurityConfig | `global/config/SecurityConfig.java` | Spring Security 설정 |
...
| AIConfig | `global/config/AIConfig.java` | Claude AI RestTemplate 설정 (NEW v1.5) |
...
| 경쟁사 비교 | `/compare` | X | 경쟁사 비교 페이지 (NEW v1.5) |
...
sequenceDiagram
    ...
    participant ReportAIService
    participant ClaudeAI
    participant DB

    User->>Frontend: AI 리포트 생성 요청
    Frontend->>ReportAPI: POST /boards/{id}/reports
    ReportAPI->>ReportAIService: 리포트 생성 요청
    ReportAIService->>DB: 보드 데이터 조회 (태스크, 피쳐, 활동 등)
    ReportAIService->>ClaudeAI: 분석 요청 (Claude API)
    ClaudeAI-->>ReportAIService: 분석 결과 반환
    ReportAIService->>DB: WeeklyReport 저장
    ReportAPI-->>Frontend: 리포트 응답
    Frontend-->>User: 리포트 표시
```
...
| features | 피쳐 카드 | id, board_id, title, description, progress (priority 제거, v1.5) |
...
| weekly_reports | AI 주간 리포트 | id, board_id, report_type, content, generated_at (NEW v1.5) |
| daily_standup_configs | 스탠드업 설정 | id, board_id, enabled, send_time, channel (NEW v1.5) |
...
- `GET /boards/{id}/reports/{reportId}` - 리포트 상세
- `POST /boards/{id}/reports/{reportId}/regenerate` - 리포트 재생성

### 데일리 스탠드업 (NEW v1.5)
- `GET /boards/{id}/standup-config` - 스탠드업 설정 조회
- `PUT /boards/{id}/standup-config` - 스탠드업 설정 수정
...
| elastic-beanstalk | EB Environment, ASG | Backend 배포 환경 (CLAUDE_API_KEY 환경변수 포함) |
...
- **변경**: `components/landing/Diagrams.tsx` - AIReportDiagram 추가
- **변경**: `components/LanguageSwitcher.tsx` - 10개 언어 지원
- **변경**: i18n 지원 언어 2개(ko, en) → 10개(+ja, zh, zh-TW, hi, vi, es, pt-BR, th)
- **추가**: 언어 파일 8개 (ja, zh, zh-TW, hi, vi, es, pt-BR, th)

### Backend
- **추가**: `domain/report/` - AI 리포트 도메인 (WeeklyReport, ReportType, ReportController, ReportService, ReportAIService, ReportRepository, DTOs)
- **추가**: `domain/standup/` - 데일리 스탠드업 도메인 (DailyStandupConfig, DailyStandupController, DailyStandupService, DailyStandupScheduler, DailyStandupConfigRepository, DTOs)
- **추가**: `global/config/AIConfig.java` - Claude AI RestTemplate 설정
- **추가**: `weekly_reports` 테이블 - AI 주간 리포트 저장
- **추가**: `daily_standup_configs` 테이블 - 스탠드업 설정
- **변경**: 도메인 패키지 24개 → 26개
- **변경**: Global 설정 파일 7개 → 8개 (AIConfig 추가)
- **변경**: Feature 엔티티에서 priority 필드 완전 제거
- **변경**: Flyway 마이그레이션 V14 → V16 (V15~V16 추가)
- **변경**: Trial 기간 7일 → 3일

### Infra
- **변경**: Elastic Beanstalk 환경 변수에 `CLAUDE_API_KEY` 추가
