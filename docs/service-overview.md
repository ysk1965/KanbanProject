...
- **이메일 인증**: 가입 후 이메일 인증 필수, 재발송 가능 (24시간 유효)
...
- **테마 설정**: 다크/라이트 모드 선택
- **언어 설정**: 10개 언어 지원 (브라우저 자동 감지)
  - ko (한국어), en (영어), ja (일본어), zh (중국어 간체), zh-TW (중국어 번체)
  - hi (힌디어), vi (베트남어), es (스페인어), pt-BR (포르투갈어), th (태국어)
- **계정 탈퇴**: 계정 영구 삭제
...
- 음성 파일 업로드 → **OpenAI Whisper API** 기반 STT 변환
- 파일 크기 제한: 최대 25MB
- 전사 텍스트 수동 편집 가능
...
- 멤버별 전일 활동 요약 포함
- **AI 요약 생성**: OpenAI 기반으로 활동 데이터를 자연어 요약하여 발송
...
|------|:---:|:---:|
| 좌석당 가격 | $5.00/seat | $50.00/seat (17% 할인) |

### 18.2 구독 상태 흐름
```
TRIAL (7일) → ACTIVE (결제 성공)
                ↓ (결제 실패)
              GRACE (3일 유예)
                ↓ (유예 만료)
              SUSPENDED (읽기 전용)
...
| 스케줄러 | 기능 |
|---------|------|
| ActivityLogCleanupScheduler | 6개월 이상 활동 로그 자동 삭제 |
| TempFileCleanupScheduler | 만료된 임시 파일 자동 삭제 |
| DailyStandupScheduler | UTC 시간 기반 일일 스탠드업 발송 (매 분 체크) |

---

## 20. Rate Limiting

### 20.1 Bucket4j 기반 요청 제한
| 대상 | 분당 제한 | 시간당 제한 |
|------|:--------:|:----------:|
| 일반 API | 300 | 3,000 |
| 로그인 (`/auth/login`) | 5 | 20 |
...
### 26.5 다국어 지원
- **i18next** 기반
- 지원 언어 (10개): 한국어, 영어, 일본어, 중국어(간체), 중국어(번체), 힌디어, 베트남어, 스페인어, 포르투갈어(브라질), 태국어
- 브라우저 언어 자동 감지 (localStorage → navigator 순서)
