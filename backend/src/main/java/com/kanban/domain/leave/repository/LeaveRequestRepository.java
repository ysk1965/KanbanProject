...
public interface LeaveRequestRepository extends JpaRepository<LeaveRequest, String> {
    ...
    @Query("SELECT lr FROM LeaveRequest lr WHERE lr.requester.id = :requesterId AND lr.status = :status")
    List<LeaveRequest> findByRequesterIdAndStatus(
            @Param("requesterId") String requesterId,
            @Param("status") LeaveStatus status);

    @Query("SELECT lr FROM LeaveRequest lr WHERE lr.policy.id = :policyId AND lr.status = :status")
