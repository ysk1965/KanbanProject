...
public class OrgNoteService {
    ...
    @Transactional
    public NoteResponse.Detail toggleLike(String orgId, String noteId, String userId) {
        organizationService.getOrgMemberOrThrow(orgId, userId);
        Note note = getNoteOrThrow(orgId, noteId);
    ...
    public NoteResponse.Detail moveNote(String orgId, String noteId, String userId, NoteRequest.Move request) {
        ...
        List<Note> siblings = (newParent != null)
                ...
                : noteRepository.findRootsByOrganizationId(orgId);
    ...
    public NoteResponse.Detail restoreVersion(String orgId, String noteId, String versionId, String userId,
        ...
        // 복원 직전 화면의 라이브 내용을 스냅샷으로 보존 (없으면 발행본).
        ...
        // 복원본이 새 발행본이 되므로 stale Yjs draft 폐기 (updateNote 발행 경로 미러).
        noteCollabService.deleteState(noteId);
        eventPublisher.publishEvent(new NoteDraftDiscardedEvent(noteId));
        eventPublisher.publishEvent(new NoteSnapshotSavedEvent(noteId));
    ...
    public List<NoteResponse.SearchResult> searchNotes(String orgId, String userId, String rawQuery) {
        organizationService.getOrgMemberOrThrow(orgId, userId);

        String query = NoteSearchSupport.normalizeQuery(rawQuery);
        if (query == null) return List.of();

        return noteRepository.searchByOrganizationId(orgId, NoteSearchSupport.likePattern(query),
    ...
    @Transactional
    public NoteResponse.Detail updateStatus(String orgId, String noteId, String userId, String rawStatus) {
        organizationService.getOrgMemberOrThrow(orgId, userId);
    ...
    @Transactional
    public NoteResponse.VersionInfo updateVersionNote(String orgId, String noteId, String versionId, String userId,
                                                      String memo) {
        organizationService.getOrgMemberOrThrow(orgId, userId);
        getNoteOrThrow(orgId, noteId);
