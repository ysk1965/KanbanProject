...
public class MemberActivityCollector {
    ...
    public MemberResult compute(String boardId, List<CommitInfo> commits,
        ...
        if (slackMessages != null && slackMessages.isArray()) {
            for (JsonNode msg : slackMessages) {
                ...
                acc.slack.add(ReportContent.MemberSlackMessage.builder()
                        .channel(text(msg, "channel_name"))
                        .text(text(msg, "text"))
                        .at(text(msg, "at"))
                        .media(harvestMedia(msg.get("files")))
                        .build());
            }
        }

        // 3) 칸반 체크리스트 — 담당자(보드 멤버)로 귀속. 완료 항목만, 태스크 제목을 맥락으로.
        for (MemberIdentity id : identities) {
            List<ChecklistItem> items;
            try {
                items = checklistItemRepository.findByBoardIdAndAssigneeId(boardId, id.userId());
            } catch (Exception e) {
                continue;
            }
            for (ChecklistItem item : items) {
                ...
                Acc acc = accs.get(id.userId());
                if (acc == null) {
                    acc = new Acc(id.name(), id.githubLogin());
                    accs.put(id.userId(), acc);
                }
